...
The following script is used to train and to evaluate a model.

Modified version of https://github.com/tensorflow/models/blob/master/research/object_detection/model_main.py
with bugfix for evaluation and some additions
...
flags.DEFINE_integer('save_checkpoints_secs', None,
                     'Secs between checkpoint saves and evaluation runs on the saved cps')
flags.DEFINE_integer('save_checkpoints_steps', None,
                     'Steps between checkpoint saves and evaluation runs on the saved cps')
flags.DEFINE_integer('keep_checkpoint_max', 10, 'Max number of last chekpoints kept')
flags.DEFINE_integer('keep_checkpoint_every_n_hours', 10000,
                     'Keep one cp every n hours (will not be deleted as the last ones)')
...
def main(unused_argv):
    ...
    if FLAGS.save_checkpoints_secs is not None:
        if FLAGS.save_checkpoints_steps is not None:
            tf.logging.warning('set both save_checkpoints_secs and save_checkpoints_steps, the latter will be ignored')
